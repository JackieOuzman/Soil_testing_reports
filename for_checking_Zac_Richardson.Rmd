...
#!!! this is longer code ?? why??
Paddock_selected_site1 <- 511022
...
check_fert_applied <- left_join(check_fert_applied,
                                paddock_details,
                                by = c("Paddock_ID" = "paddock6_digits"))
...
check_fert_applied <-distinct_at(check_fert_applied, vars(`Strip Rate`, `Strip Type`), .keep_all = TRUE)

check_fert_applied <- check_fert_applied %>% arrange(`Paddock ID`, `Strip Type`,`P content`)
...
high_low_comp_define <-distinct(high_low_comp, paddock_ID, Strip_Type, .keep_all = TRUE  ) %>% 
  dplyr::select(`paddock ID` = paddock_ID,
                Farmer,
                `Paddock tested`,
                Strip_Type,
...
#remove the NA 
GR_comparison$Significant <- as.character(GR_comparison$Significant)
GR_comparison <- GR_comparison %>% 
...
recom_rateDB <- recom_rateDB %>% 
  dplyr::mutate(paddock6_digits = substr(Zone_ID, start = 1, stop = 6))

#select only the paddocks we are working with
recom_rateDB <- recom_rateDB %>%
  dplyr::filter(
    paddock6_digits == Paddock_selected_site1 )
    # paddock5_digits == Paddock_selected_site2 |
    # paddock5_digits == Paddock_selected_site3 | 
    # paddock5_digits == Paddock_selected_site4 |
    # paddock5_digits == Paddock_selected_site5 |
...
recom_rateDB <- recom_rateDB %>% 
  distinct(`Zone ID`, .keep_all = TRUE)
