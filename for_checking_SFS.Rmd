...
site10 <- paste0((unique(paddock_done2020$Farmer)[5]),"/",
                  (unique(paddock_done2020$`Paddock tested`)[10]),
                  "/output/")
...
yld_trace_site10 <- list.files(path = paste0(Growers_folder, site10),pattern = "trace(.*)$", ignore.case = TRUE)
...
harvest_map_site10 <- list.files(path = paste0(Growers_folder, site10),pattern = "Final(.*)$", ignore.case = TRUE)
### paddock id details
...
site_name_10 <- paste0("^",paddock_done2020$paddock[10])
...
Paddock_selected_site10 <- paddock_done2020$paddock[10]
...
image_read(dir(png_folder, full.names=T, pattern=Paddock_selected_site8)) %>%
  image_rotate(90)
...
image_read(dir(png_folder, full.names=T, pattern=Paddock_selected_site9)) %>%
  image_rotate(90)
...
```{r display site 10, echo=FALSE, message=FALSE, warning=FALSE}


include_graphics(paste0(Growers_folder,site10,  harvest_map_site10))
```
...
```{r "display site 10 yld trace", echo=FALSE, message=FALSE, warning=FALSE}


include_graphics(paste0(Growers_folder,site10,  yld_trace_site10))
```

### Results

```{r "display results site 10 page 1", echo=FALSE, message=FALSE, warning=FALSE}



image_read(dir(png_folder, full.names=T, pattern=Paddock_selected_site10)) %>%
  image_rotate(90)
