...
##########################################################################################################################
#End of function call it and then user input required

df_step1 <- function_filter_data(comparison, Strip_type, high_low_comp_t) 
#assign(paste0(quo(comparison), "_", substr(Strip_type, start = 1, stop = 1)), missing stuff
names(df_step1)

##!!!! user input is required to select clms needed for analysis
comparison
df_step1 <- df_step1 %>%
