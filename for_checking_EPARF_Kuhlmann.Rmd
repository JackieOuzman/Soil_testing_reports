...
- Most of these strips are very narrow and don't contain much yield data.
- We have a standard protocol to remove segemnets that don't conatin much yield data. 
  However if I follow this protocol I would remove most of the results.
  Therefore I have left all the data in.
- There is no recommned N or P rates for paddock 33, I have emailed Sean asking for this info.
- None of these padocks have an Alt GSP strip.
...
check_fert_applied <-distinct_at(check_fert_applied, vars(`Strip Rate`,`Paddock tested`, `Strip Type`), .keep_all = TRUE)

check_fert_applied <- check_fert_applied %>% arrange(`Paddock ID`, `Paddock tested`, `Strip Type`, `Strip Rate`)
...
high_low_comp_N <- high_low_comp %>%
  dplyr::filter(Strip_Type == "N Strip")
high_low_comp_P <- high_low_comp %>%
...
high_low_comp_N_rates <- dplyr::distinct(high_low_comp_N, Zone_ID, .keep_all = TRUE)

high_low_comp_N_rates <- high_low_comp_N_rates %>% 
  dplyr::select(`Zone ID` = Zone_ID,
...
high_low_comp_P_rates <- dplyr::distinct(high_low_comp_P, Zone_ID, .keep_all = TRUE)

high_low_comp_P_rates <- high_low_comp_P_rates %>% 
  dplyr::select(`Zone ID` = Zone_ID,
...
## big table with rates used


high_low_comp_rates <- rbind(high_low_comp_P_rates, high_low_comp_N_rates)
#high_low_comp_rates <- arrange(high_low_comp_rates,`paddock ID`, `Strip Type` )

#using the padock code and zone ID
paddock_details_join <- distinct(paddock_details, `Paddock code`, .keep_all = TRUE)
paddock_details_join <- paddock_details %>%  dplyr::select(`Zone ID`=`Paddock code`, `Paddock tested`,  `Zone name` =  Zone )

paddock_details_join <- as.data.frame(paddock_details_join)
high_low_comp_rates$`Zone ID`<- as.numeric(high_low_comp_rates$`Zone ID`)

high_low_comp_rates <- dplyr::left_join(high_low_comp_rates, paddock_details_join, by = c("Zone ID" = "Zone ID" ))
 

high_low_comp_rates <- dplyr::mutate(high_low_comp_rates, unique_ID = paste0(`Zone ID`, '_', `Strip Type`)) %>% 
...
high_low_comp <- dplyr::mutate(high_low_comp, unique_ID = paste0(Zone_ID, '_', Strip_Type, comparison)) %>% 
  dplyr::distinct(unique_ID , .keep_all = TRUE) %>% 
  dplyr::select(-unique_ID)
...
paddock_details_join <- paddock_details_join %>%
  dplyr::filter(
      paddock5_digits == Paddock_selected_site1 |
      paddock5_digits == Paddock_selected_site2 |
      paddock5_digits == Paddock_selected_site3 |
      paddock5_digits == Paddock_selected_site4 
    # paddock_ID == Paddock_selected_site5
  )
...
high_low_comp$Zone_ID<- as.numeric(high_low_comp$Zone_ID)
high_low_comp <- dplyr::left_join(high_low_comp, paddock_details_join, by = c("Zone_ID" = "Zone ID" ))
...
high_low_comp_high_only <- high_low_comp %>% 
  filter(comparison == "high fert vs low fert") %>%  
  dplyr::select('Zone ID' = Zone_ID,
                `zone name` = Zone ,
               # Farmer,
                `Paddock tested`,
                ...
                "yield response" = yld_response,
                `Significant practical` = Significant_practical) 
high_low_comp_high_only <- high_low_comp_high_only %>% 
  arrange(`Zone ID`, `Strip Type`)



high_low_comp_high_only %>% 
  formattable(align = "l", - 1  )
...
high_v_med_comp_results <- high_low_comp %>% 
  filter(comparison == "high fert vs medium fert") %>%  
  dplyr::select('Zone ID' = Zone_ID,
                `zone name` = Zone ,
                # Farmer,
                `Paddock tested`,
...
med_v_low_comp_results <- high_low_comp %>% 
  filter(comparison == "medium fert vs low fert") %>%  
  dplyr::select('Zone ID' = Zone_ID,
                `zone name` = Zone ,
                # Farmer,
                `Paddock tested`,
...
temp <- paddock_details %>% 
  dplyr::select("Paddock code", "Farmer", "Paddock tested", `Zone name` = "Zone" ) %>% 
  dplyr::distinct(`Paddock code`, .keep_all = TRUE)

GR_comparison <- left_join(GR_comparison, temp, by= c("Zone_ID" = "Paddock code"))

# ##oops I have a problem with duplicate names
# GR_comparison <- GR_comparison %>% 
#   rename(`Paddock tested` = `Paddock tested.x`,
#          Farmer = Farmer.x)
...
GR_comparison_higher_rates <- GR_comparison %>% 
  ...
  arrange(`Paddock tested`, `Strip Type`)
...
```{r rec rate comp high low plot N, echo=FALSE, message=FALSE, warning=FALSE}
...
```{r rec rate comp high table rates N, echo=FALSE, message=FALSE, warning=FALSE}
